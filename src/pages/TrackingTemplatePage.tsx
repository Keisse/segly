...
const TrackingTemplatePage = () => {
  ...
  const handleExportExcel = () => {
    ...
    try {
      ...
      const mainData = [["TEMPLATE DE ACOMPANHAMENTO DA EXECUÇÃO"], [""], ["Pilar", selectedPillarData.name], ["Nível de Maturidade", `${selectedStageData.label} (${selectedStageData.range})`], [""], ["LINHA DO TEMPO"], ["Data de Início", format(startDate, "dd/MM/yyyy", {
        locale: ptBR
      })], ["Data de Revisão", `${format(calculatedDates.reviewDate, "dd/MM/yyyy", {
        locale: ptBR
        ...
        locale: ptBR
      })} até ${format(calculatedDates.completionDate, "dd/MM/yyyy", {
        locale: ptBR
      })}`], ["Status", getStatusText(collectiveTask.status)], ["Ação Principal", templateData.acao_coletiva], [""], ["Subtarefas Coletivas", "Status"], ...collectiveTask.subtasks.map(st => [st.text, getStatusText(st.status)]), ...(collectiveTask.subtasks.length === 0 ? [["(Nenhuma subtarefa adicionada)", ""]] : []), [""], ["INDICADOR DE SUCESSO"], ["", templateData.indicador_sucesso], ["Autoavaliação", getEvaluationText()], [""], ["TRILHA RECOMENDADA"], ["Curso", `${templateData.curso_codigo}: ${templateData.curso_nome}`], [""], [""], ["Gerado pelo Diagnóstico de Alta Performance — Allevo For Business", format(new Date(), "dd/MM/yyyy", {
        locale: ptBR
      })]];
  ...
  return <div className="min-h-screen bg-background py-8 px-4">
      ...
      <Card className="bg-card/50 border-border/50 backdrop-blur-sm">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                <Label>2. Pilar</Label>
                <Select value={selectedPillar?.toString() || ""} onValueChange={value => setSelectedPillar(Number(value))}>
                  <SelectTrigger className="bg-background/50">
                    <SelectValue placeholder="Selecione" />
                  </SelectTrigger>
                  <SelectContent>
                    {pillars.map(pillar => <SelectItem key={pillar.id} value={pillar.id.toString()}>
                        {pillar.name}
                      </SelectItem>)}
                  </SelectContent>
                </Select>
              ...
              <Button onClick={handleGenerate} className="w-full" disabled={!selectedStage || !selectedPillar}>
                Gerar Template
                <ArrowRight className="w-4 h-4 ml-2" />
              </Button>
            </CardContent>
          </Card>) : templateData && calculatedDates && selectedPillarData && selectedStageData ? (/* Generated Template */
      <div className="space-y-6">
              ...
              <CardContent className="p-6 space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-card/80 rounded-lg p-4 border border-border/50 space-y-4">
                    ...
                    <div className="pl-6 space-y-2">
                      ...
                      {individualTask.subtasks.map(subtask => <div key={subtask.id} className="flex items-center gap-2">
                          <span className={cn("text-sm flex-1", subtask.status === "completed" && "line-through text-muted-foreground")}>
                            {subtask.text}
                          </span>
                          <Select value={subtask.status} onValueChange={value => updateSubtaskStatus("individual", subtask.id, value as TaskStatus)}>
                      ...
                      <div className="flex gap-2">
                        <input type="text" placeholder="Adicionar subtarefa..." value={newSubtaskIndividual} onChange={e => setNewSubtaskIndividual(e.target.value)} onKeyPress={e => e.key === "Enter" && addSubtask("individual")} className="flex-1 bg-background/50 border border-border rounded px-2 py-1 text-sm" />
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-card/80 rounded-lg p-4 border border-border/50 space-y-4">
                    ...
                    <div className="pl-6 space-y-2">
                      <div className="text-xs text-muted-foreground">Subtarefas:</div>
                      {collectiveTask.subtasks.map(subtask => <div key={subtask.id} className="flex items-center gap-2">
                          <span className={cn("text-sm flex-1", subtask.status === "completed" && "line-through text-muted-foreground")}>
                            {subtask.text}
                          </span>
                          <Select value={subtask.status} onValueChange={value => updateSubtaskStatus("collective", subtask.id, value as TaskStatus)}>
                            <SelectTrigger className="w-36 h-7 text-xs bg-background/50">
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                      ...
                      <div className="flex gap-2">
                        <input type="text" placeholder="Adicionar subtarefa..." value={newSubtaskCollective} onChange={e => setNewSubtaskCollective(e.target.value)} onKeyPress={e => e.key === "Enter" && addSubtask("collective")} className="flex-1 bg-background/50 border border-border rounded px-2 py-1 text-sm" />
                  ...
                  <div className="bg-card/80 rounded-lg p-4 border border-border/50 space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <Button size="sm" variant={successEvaluation === "achieved" ? "default" : "outline"} onClick={() => setSuccessEvaluation("achieved")} className={cn(successEvaluation === "achieved" && "bg-emerald-600 hover:bg-emerald-700 text-white")}>
                          ✅ Atingido
                        </Button>
                        <Button size="sm" variant={successEvaluation === "partial" ? "default" : "outline"} onClick={() => setSuccessEvaluation("partial")} className={cn(successEvaluation === "partial" && "bg-amber-600 hover:bg-amber-700 text-white")}>
                          ⚠️ Parcialmente
                        </Button>
                        <Button size="sm" variant={successEvaluation === "not_achieved" ? "default" : "outline"} onClick={() => setSuccessEvaluation("not_achieved")} className={cn(successEvaluation === "not_achieved" && "bg-destructive hover:bg-destructive/90 text-destructive-foreground")}>
                          ❌ Não atingido
                        </Button>
                      </div>
                    </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Button onClick={handleExportExcel} disabled={isGeneratingPDF} className="bg-primary hover:bg-primary/90 text-primary-foreground">
                {isGeneratingPDF ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <FileSpreadsheet className="w-4 h-4 mr-2" />}
                Gerar Template
              </Button>
