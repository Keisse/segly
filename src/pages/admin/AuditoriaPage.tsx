  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      const haystack = [
        ...
        row.lead?.nome,
        row.lead?.empresa,
        ...row.changed_fields.map((f) => fieldLabels[f] || f),
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Histórico completo de alterações em leads e atividades. Visível apenas para líderes e administradores.</p>
      </div>

      <div className="flex flex-col gap-3 lg:flex-row">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar por usuário, lead, empresa ou campo alterado" className="pl-9" />
        </div>
      ...
      {isLoading ? (
        ...
        <Card><CardContent className="p-8 text-center text-sm text-muted-foreground">Nenhum evento encontrado.</CardContent></Card>
      ) : (
              ...
              <CardContent className="p-4 space-y-3">
                <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                  <div className="space-y-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <Badge variant="outline">{row.entity_type === "lead" ? "Lead" : "Atividade"}</Badge>
                      {row.lead_id && (
                        <Link to={`/admin/lead/${row.lead_id}`} className="font-semibold hover:text-primary truncate">
                          {row.lead?.empresa || row.lead?.nome || "Abrir lead"}
                        </Link>
                      )}
                    </div>
                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                      <span className="inline-flex items-center gap-1"><User className="h-3.5 w-3.5" />{row.actor?.display_name || "Sistema / origem externa"}</span>
                ...
                {row.action === "update" && row.changed_fields.length > 0 && (
                  <div className="rounded-lg border bg-muted/20 overflow-hidden">
                    <div className="px-3 py-2 border-b flex items-center gap-2 text-sm font-medium"><FileDiff className="h-4 w-4" />Campos alterados</div>
                    <div className="divide-y">
                      {row.changed_fields.map((field) => (
                        <div key={field} className="grid gap-1 p-3 md:grid-cols-[180px_1fr_1fr] text-sm">
                          <div className="font-medium">{fieldLabels[field] || field}</div>
                          <div><span className="text-xs text-muted-foreground block">Antes</span><span className="break-words">{formatValue(row.old_data?.[field])}</span></div>
                          <div><span className="text-xs text-muted-foreground block">Depois</span><span className="break-words">{formatValue(row.new_data?.[field])}</span></div>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {row.action !== "update" && (
                  <div className="text-sm text-muted-foreground inline-flex items-center gap-2">
                    ...
                    {row.action === "insert" ? "Registro criado no sistema." : "Registro removido do sistema."}
                  </div>
                )}
              </CardContent>
            </Card>
