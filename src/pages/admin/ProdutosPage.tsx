...
export default function ProdutosPage() {
  ...
  const edit = (product: ProductRow) => {
    ...
    setForm({
      ...
      broker_commission_pct: String(product.broker_commission_pct ?? 0),
      notes: product.notes || "",
    });
    setOpen(true);
  };
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      if (!Number.isFinite(brokerPct) || brokerPct < 0 || brokerPct > 100) throw new Error("Comissão do corretor deve estar entre 0 e 100%.");
    ...
    <div className="p-4 sm:p-6 max-w-6xl mx-auto space-y-6 min-w-0">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Cadastre cada produto e defina as porcentagens da administradora e do corretor.</p>
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <Card><CardContent className="p-4"><p className="text-xs text-muted-foreground">Regra</p><p className="text-sm font-semibold mt-1">Comissão configurada por produto</p></CardContent></Card>
      </div>
        ...
        <CardContent className="space-y-3">
          {isLoading ? <p className="text-sm text-muted-foreground">Carregando produtos...</p> : products.length === 0 ? (
            ...
            <div key={product.id} className="grid gap-3 rounded-xl border p-4 lg:grid-cols-[1.4fr_.8fr_.8fr_.8fr_auto] lg:items-center">
              ...
              <div><p className="text-xs text-muted-foreground">Administradora</p><p className="font-semibold">{Number(product.admin_commission_pct).toLocaleString("pt-BR", { maximumFractionDigits: 3 })}%</p></div>
              <div><p className="text-xs text-muted-foreground">Corretor</p><p className="font-semibold">{Number(product.broker_commission_pct).toLocaleString("pt-BR", { maximumFractionDigits: 3 })}%</p></div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-xl">
          <DialogHeader><DialogTitle>{editing ? "Editar produto" : "Cadastrar produto"}</DialogTitle><DialogDescription>Essas porcentagens serão usadas como regra de comissão quando a venda for vinculada ao produto.</DialogDescription></DialogHeader>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-1.5"><Label>Comissão da administradora (%) *</Label><Input inputMode="decimal" value={form.admin_commission_pct} onChange={(e) => setForm({ ...form, admin_commission_pct: e.target.value })} /></div>
            <div className="space-y-1.5"><Label>Comissão do corretor (%) *</Label><Input inputMode="decimal" value={form.broker_commission_pct} onChange={(e) => setForm({ ...form, broker_commission_pct: e.target.value })} /></div>
