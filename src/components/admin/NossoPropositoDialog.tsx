...
export function NossoPropositoDialog({ open, onOpenChange }: NossoPropositoDialogProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[calc(100vw-1rem)] max-w-xl bg-white text-slate-800 border-0 shadow-2xl rounded-2xl p-0 max-h-[calc(100vh-1rem)] overflow-hidden">
        <div className="overflow-y-auto max-h-[calc(100vh-1rem)] px-5 py-7 sm:px-10 sm:py-12">
          <DialogHeader className="items-center text-center space-y-4">
            <div
              className="flex items-center justify-center h-14 w-14 rounded-full mx-auto shrink-0"
              style={{ backgroundColor: "rgba(47, 177, 132, 0.1)" }}
            >
              <Compass size={28} strokeWidth={1.75} style={{ color: "#2FB184" }} />
            </div>
            <DialogTitle className="text-xl sm:text-3xl font-semibold tracking-tight text-slate-900">
              Mais do que organizar vendas
            </DialogTitle>
            <DialogDescription className="text-sm sm:text-[15px] leading-relaxed text-slate-600 text-left">
              O Segly existe para ajudar pessoas e negócios a crescerem com clareza,
              responsabilidade e cuidado.
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-5 text-sm sm:text-[15px] leading-relaxed text-slate-600 text-left mt-6">
            <p>
          ...
          <p
            className="mt-8 text-center text-base sm:text-xl font-semibold"
            style={{
              color: "#1A9A70",
          ...
          <div className="mt-8 sm:mt-10 flex justify-center">
            <Button
              ...
              className="w-full sm:w-auto px-8 h-11 rounded-full text-white hover:opacity-90 transition-opacity"
