...
export function StageTransitionDialog({ open, onOpenChange, lead, stageId, stageName, onConfirm }: Props) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      fields.forEach((field) => {
        const raw = saved[field.field_key];
        initial[field.field_key] = raw == null ? "" : String(raw);
      });
      setValues(initial);
  ...
  }, [open, lead, stageId, fields, isImplanted]);
  ...
  const moveLead = async (completeNow: boolean) => {
    ...
    try {
      ...
      await onConfirm();
      onOpenChange(false);
      toast.success(completeNow ? "Informações salvas e vida movida." : "Respostas parciais salvas. Você pode completar depois.");
    } catch (error) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto"><DialogHeader><DialogTitle>Informações de {stageName}</DialogTitle><DialogDescription>Estas informações não bloqueiam a movimentação. Preencha o que souber agora, deixe o restante em branco e complete depois.</DialogDescription></DialogHeader>
    ...
    <DialogFooter className="gap-2 sm:gap-2"><Button variant="ghost" onClick={() => onOpenChange(false)} disabled={saving}>Cancelar</Button><Button variant="outline" onClick={() => moveLead(false)} disabled={saving || isLoading}>{saving && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}Salvar e responder depois</Button><Button onClick={() => moveLead(true)} disabled={saving || isLoading || (isImplanted && productsLoading)}>{saving && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}Salvar e mover vida</Button></DialogFooter>
