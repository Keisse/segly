...
const LeadCaptureForm = ({
  ...
  return <motion.form initial={{
    ...
    delay: 0.3
  }} onSubmit={handleSubmit} className="w-full max-w-md space-y-5">
      {/* Nome */}
      <div className="space-y-2">
        <Label htmlFor="nome" className="text-foreground font-medium">
          Nome<span className="text-primary">*</span>
        </Label>
        <Input id="nome" placeholder="Seu nome completo" value={formData.nome} onChange={e => handleInputChange("nome", e.target.value)} className="bg-card border-border/50 focus:border-primary" />
      ...
      <div className="space-y-2">
        ...
        <Input id="telefone" placeholder="(11) 99999-9999" value={formData.telefone} onChange={e => handleInputChange("telefone", e.target.value)} className="bg-card border-border/50 focus:border-primary" />
      ...
      <div className="space-y-2">
        ...
        <Input id="email" type="email" placeholder="seuemail@empresa.com.br" value={formData.email} onChange={e => handleInputChange("email", e.target.value)} className="bg-card border-border/50 focus:border-primary" />
      ...
      <div className="space-y-2">
        ...
        <Input id="empresa" placeholder="Nome da empresa" value={formData.empresa} onChange={e => handleInputChange("empresa", e.target.value)} className="bg-card border-border/50 focus:border-primary" />
      ...
      <div className="space-y-2">
        ...
        <Select value={formData.porte} onValueChange={value => handleInputChange("porte", value)}>
          ...
          <SelectContent className="bg-popover border-border shadow-lg z-[100]">
            {porteOptions.map(option => <SelectItem key={option} value={option} className="hover:bg-accent focus:bg-accent cursor-pointer">
                {option}
              </SelectItem>)}
          </SelectContent>
        </Select>
      ...
      <div className="space-y-2">
        ...
        <Select value={formData.departamento} onValueChange={value => handleInputChange("departamento", value)}>
          ...
          <SelectContent className="bg-popover border-border shadow-lg z-[100]">
            {departamentoOptions.map(option => <SelectItem key={option} value={option} className="hover:bg-accent focus:bg-accent cursor-pointer">
                {option}
              </SelectItem>)}
          </SelectContent>
        </Select>
      ...
      <div className="space-y-2">
        ...
        <Select value={formData.cargo} onValueChange={value => handleInputChange("cargo", value)}>
          ...
          <SelectContent className="bg-popover border-border shadow-lg z-[100]">
            {cargoOptions.map(option => <SelectItem key={option} value={option} className="hover:bg-accent focus:bg-accent cursor-pointer">
                {option}
              </SelectItem>)}
          </SelectContent>
        </Select>
